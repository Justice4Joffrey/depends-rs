...
fn derive_graph_inner(input: TokenStream) -> syn::Result<TokenStream> {
    ...
    let constraint = quote! {
        for<'a> R: ::depends::core::Resolve<Output<'a> = ::std::cell::Ref<'a, ::depends::core::NodeState<#root_type>>> + 'a,
    };
    ...
    Ok(quote! {
        mod #module_name {
            ...
            pub struct #graph_ident<R> {
                #(#input_idents: ::std::rc::Rc<::depends::core::InputNode<#input_types>>,)*
                #root_var_name: R
            }
            ...
            where
                #constraint
            {
                fn new(#(#input_idents: ::std::rc::Rc<::depends::core::InputNode<#input_types>>,)* #root_var_name: R) -> Self {
                    Self {
                        #(#input_idents,)*
                        #root_var_name
                    }
                ...
                #(pub fn #fn_names(&self, update: <#input_types as ::depends::core::UpdateInput>::Update) -> ::depends::core::error::ResolveResult<()> {
                    self.#input_idents.update(update)
                })*
            }

            impl<R> ::depends::core::Resolve for #graph_ident<R>
            where
                #constraint
            {
                type Output<'a> = <R as ::depends::core::Resolve>::Output<'a> where Self: 'a;
            ...
            impl #ident {
                ...
                ) -> #module_name :: #graph_ident<impl for<'a> ::depends::core::Resolve<Output<'a> = ::std::cell::Ref<'a, ::depends::core::NodeState<#root_type>>>> {
                    #(let #input_idents = ::depends::core::InputNode::new(#input_idents);)*
                    #(#derive_constructors)*
            ...
            impl ::depends::core::GraphCreate for #ident {
...
fn derived_node_create(derived: &DerivedNodeDef) -> (TokenStream, Ident, Ident) {
    ...
    let deps_init = if let Some(dep_type) = derived.dependency_type.as_ref() {
        ...
        quote! {
            #(::depends::core::Dependency::new(::std::rc::Rc::clone(& #deps)))*
        }
    };
    (
        quote! {
            let #ident = ::depends::core::DerivedNode::new(
                #deps_init,
                #operation,
                #ident
            );
